...
static int semi(void){
    printk("\n inside them semaphore driver\n");
    alloc_chrdev_region(&dev,0,1,"sem_c");
    printk("\n major:%d and minor:%d\n",MAJOR(dev),MINOR(dev));
    myclass= class_create(THIS_MODULE,"sem_dr");
    device_create(myclass,NULL,dev,NULL,"sem_dr");
    cdev_init(&mydev,&fs);
    cdev_add(&mydev,dev,1);
    sema_init(&my_sem,1);   //only one shared resource critical section which is result. 
    return 0;
}
...
static void seme(void){
    ...
    printk("\n exit from the semaphore driver\n");
